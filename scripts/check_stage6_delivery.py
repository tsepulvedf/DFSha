...
def main():
    ...
    for relative in sorted(paths):
        if relative == 'docs/evidencias/etapa6/delivery-review.json':
    ...
    result = subprocess.run(['git', 'diff', '--check', 'HEAD'], cwd=ROOT, capture_output=True, text=True)
    if result.returncode:
        failures.append('git diff --check')
    evidence = ROOT / 'docs/evidencias/etapa6/delivery-review.json'
