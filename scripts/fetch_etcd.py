...
def main() -> None:
    ...
    parser.add_argument('--tools', action='store_true', help='Incluir etcdctl y etcdutl para respaldo/restauración')
    ...
    for name in (('etcd', 'etcdctl', 'etcdutl') if args.tools else ('etcd',)):
        binary_name = name + ('.exe' if target_platform.startswith('windows') else '')
        ...
        if archive.suffix == ".zip":
            ...
            with tarfile.open(archive, "r:gz") as source:
                ...
                payload = member.read()
        # Solo miembros con nombre fijo del archivo cuyo SHA fue verificado.
