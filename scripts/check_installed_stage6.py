"""R3/W2 desde el wheel instalado, sin editable ni acceso del cliente a datos DN."""
import hashlib
import json
...
def main():
    ...
    root = ROOT / '.runtime/installed-e6' / str(uuid4())
    with Cluster(root, replication=True) as lab:
        client = lab.client()
        try:
            source = root / 'source'
            ...
            def protected():
                state = client.protection('/installed')
                return state if all(b.eligible == 3 for b in state.blocks) else None
            protection = wait_until(protected, seconds=90)
            assert protection.minimum_durable == 2
            client.close(writer)
            client.close(reader)
            report = dict(status='EJECUTADO', python=sys.executable, package=dfsha.__file__,
                ...
                datanode_pids=[x.info['pid'] for x in lab.nodes[:3]], snapshots_verified=True, target_replicas=3, minimum_durable=2)
        finally:
            client.shutdown()
    evidence = ROOT / 'docs/evidencias/etapa6/installed-smoke.json'
