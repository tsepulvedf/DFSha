...
def serve(config_path: Path, ready_file: Path | None = None, stop_file: Path | None = None) -> None:
    cfg = read_config(config_path)["server"]
    certs = Path(cfg["certificate_dir"])
    stop = threading.Event()
    for sig in (signal.SIGINT, signal.SIGTERM):
        signal.signal(sig, lambda *_: stop.set())
    servers, pools = [], []
    ready = {"pid": os.getpid(), "filesystem_implemented": False}
    try:
        for listener, internal in (("public", False), ("internal", True)):
            ...
            register(server, internal)
            add_DiagnosticServiceServicer_to_server(Diagnostic(listener), server)
            server.start()
